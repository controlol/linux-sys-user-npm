...
var validUsernameRegex = /^([a-z0-9_-]{0,30})$/;
...
var user_arg_map = {
  ...
  home_dir: function(path){
    if(path){
      return util.format('--home "%s"', path);
    }
  },
  ...
  uid: function(uid){
    if (uid){
      return util.format('--uid %d', uid)
    }
  },
  gid: function(gid){
    if (gid){
      return util.format('--gid %d', gid)
    }
  },
  create_group: function(isSet){
    if (isSet){
      return "-U"
    }
  }
...
exports.addSSHtoUser = function (user, key, callback){
  exports.verifySSHKey(key, function(error, info){
    ...
    exports.getUserInfo(user, function(error, info){
      ...
      var commands = [
        util.format('mkdir /home/%s/.ssh;', user),
        util.format('touch /home/%s/.ssh/authorized_keys;', user),
        util.format('chown 700 /home/%s/.ssh;', user),
        util.format('chmod 600 /home/%s/.ssh/authorized_keys;', user),
        util.format('chown %s:%s /home/%s/.ssh -R;', user, user, user),
        util.format('echo "%s" >> /home/%s/.ssh/authorized_keys;', key, user)
      ].join(' ');
